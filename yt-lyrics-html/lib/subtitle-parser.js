...
const SubtitleParser = (function() {
    ...
    // 字幕行解析正則表達式（支援拼音欄位和角色欄位）
    // 格式: Line 1 | Word 1 | 00:00:00 → 00:00:00 | 字 | 拼音 | 角色
    // 角色只能是 1, 2, 3
    const SUBTITLE_LINE_REGEX = /Line (\d+) \| Word (\d+) \| (\d{2}):(\d{2}):(\d{2}) → (\d{2}):(\d{2}):(\d{2}) \| ([^|]+?)(?:\s*\|\s*([^|]*?))?(?:\s*\|\s*([123]))?$/;
    ...
    function parseSubtitleLine(line) {
        ...
        // 解析拼音（如果有，空字串視為 null）
        const pinyinRaw = match[10] ? match[10].trim() : null;
        const pinyin = pinyinRaw && pinyinRaw !== '' ? pinyinRaw : null;
        // 解析角色（如果有）
        const role = match[11] ? match[11].trim() : null;
