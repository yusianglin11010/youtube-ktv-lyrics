...
const GroupMapping = (function () {
  ...
  function openDialog() {
    ...
    if (
      ...
      MakerState.pinyinToLyricMappings.length > 0
    ) {
      if (
        !confirm(
    ...
    let pinyinHTML = pinyinList
      .map((syllable, idx) => {
        ...
        return `<div class="${classes.join(
          " "
        )}" data-idx="${idx}">${syllable}</div>`;
      })
      .join("");

    // Render lyrics list with color indicators
    let lyricsHTML = lyricsList
      .map((char, idx) => {
        ...
        if (mapping) {
          ...
          return `<div class="${classes.join(
            " "
          )}" data-idx="${idx}">${pinyinAnnotation}<div class="lyric-char">${char}</div></div>`;
        }

        return `<div class="${classes.join(
          " "
        )}" data-idx="${idx}">${char}</div>`;
      })
      .join("");
  ...
  function getMappingProgress(line) {
    ...
    let lineInfo = `<span>第 ${line + 1} 行 / 共 ${
      state.pinyinData.length
    } 行</span>`;
  ...
  function isMappedPinyin(line, idx) {
    return state.mappings.some(
      (m) => m.line === line && m.pinyinIndices.includes(idx)
    );
  }

  function isMappedLyric(line, idx) {
    return state.mappings.some((m) => m.line === line && m.lyricIdx === idx);
  }
    ...
    keyboardHandler = (e) => {
      ...
      if (e.key === "ArrowLeft") {
        ...
        if (state.pinyinFocus > 0) {
          ...
          // 在第一個拼音時按左鍵,跳到上一行的最後一個拼音
          prevLine();
          state.pinyinFocus = state.pinyinData[state.currentLine].length - 1;
          renderInterface();
        }
        ...
        if (state.pinyinFocus < pinyinList.length - 1) {
          state.pinyinFocus++;
          renderInterface();
        } else if (state.currentLine < state.pinyinData.length - 1) {
          // 在最後一個拼音時按右鍵,跳到下一行的第一個拼音
          nextLine();
          // pinyinFocus 已經在 nextLine() 中重置為 0
        }
      } else if (e.key === " ") {
        e.preventDefault();
  ...
  function createMapping() {
    ...
    // 檢查當前行是否已完成,若完成則自動跳到下一行
    let validation = validateLineMapping(line);
    if (
      validation.complete &&
      state.currentLine < state.pinyinData.length - 1
    ) {
      // 延遲一點時間讓使用者看到完成狀態
      setTimeout(() => {
        nextLine();
  ...
  function nextLine() {
    ...
    if (!validation.complete) {
      if (
        !confirm(
          `第 ${state.currentLine + 1} 行尚未完成 mapping：\n${
            validation.message
          }\n\n是否仍要繼續到下一行？`
        )
      ) {
  ...
  function validateLineMapping(line) {
    ...
    if (mappedPinyin.size < pinyinList.length) {
      return {
        complete: false,
        message: `還有 ${
          pinyinList.length - mappedPinyin.size
        } 個拼音音節未 mapping`,
      };
    }
    ...
    if (mappedLyrics.size < lyricsList.length) {
      return {
        complete: false,
        message: `還有 ${
          lyricsList.length - mappedLyrics.size
        } 個主歌詞字元未 mapping`,
      };
    }
  ...
  function generateGroupedPinyinText() {
    ...
    for (let lineIdx = 0; lineIdx < state.pinyinData.length; lineIdx++) {
      ...
      let lineGroups = lineMappings.map((m) => {
        let syllables = m.pinyinIndices.map(
          (idx) => state.pinyinData[lineIdx][idx]
        );
  ...
  function mergePinyinTimestamps() {
    ...
    mergeMap.forEach((mergeInfo) => {
      let groupEntries = mergeInfo.pinyinIndices
        .map((syllableIdx) => {
          return MakerState.pinyinTimestamps.find(
            (p) => p.line === mergeInfo.line && p.syllableIndex === syllableIdx
          );
        })
        .filter(Boolean);
